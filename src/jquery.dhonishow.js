		...
		return this.each( function ( index ) {
			...
			var Dom = {
				...
				wrapImages: function(){
					...
					console.log(Effect["appear"].Dom.wrapImages(ol, $(parentElement).children()));
			...
			Dom.elements = {
				...
				update: function () {
					if(options.get("hide").navigation) {this.navigation.hide(); return;}
					this.updatePaging();
				...
				updateButtons: function () {
					...
					if( options.get("hide").buttons ) 	return;
				...
				updatePaging: function () {
					...
					if(options.get("hide").paging){ this.paging.hide(); return;}
					this.paging.text(Template.evaluate(options.get("pagingTemplate"), {
				...
				updateAlt: function ( element ) {
					if(options.get("hide").alt){ this.alt.hide(); return; }
				...
				changeImage: function ( next, current ) {
					this.update();
					changingImages = true;
					if(options.get("duration") == "0") options.set("duration", 0.001); // jQuery's animate function takes only durations above zero

					if(options.get("resize")) {
						var dimensions = Dimensions.give(el = dhonishowElements[current]);

						$(parentElement).find(".dhonishow-images").animate({
							width: dimensions.width,
							height: dimensions.height
						}, options.get("duration")*1000);
						$(parentElement).animate({width: dimensions.width}, options.get("duration")*1000);
					}
					
					Effect.api.changeImage.animate(dhonishowElements[next], dhonishowElements[current]);
					Effect.api.changeImage.after();
				}
			};
			...
			var Effect = {
				api: {
					changeImage: {
						initialize: function(next, current){
							Dom.elements.update();
							changingImages = true;
							
							Effect[options.get("effect_name")].animate(next, current);
			...
			$.extend(Effect, {
				appear : {
					...
					Dom: {
						wrapImages: function(parent, elements){
							elements.each(function(){
							});
						}
					},
					animate: function(next, current){
						current.animate(this.options, options.duration*1000);
						next.animate(this.options, options.duration*1000);
					},
					resize: function(){}
				},
				vertical: $.extend(Effect.appear, {options:{height:"toggle"}}),
				horizontal: $.extend(Effect.appear, {options:{width:"toggle"}})
			});

			var Autoplay = {
				initialize: function () {
					if(options.get("autoplay")) this.periodicalExecuter = setInterval(this.periodical, options.get("autoplay")*1000);
				},
			...
			$.extend(options, {
				set: function (names, val) {
					if(!arguments[1]){
						!parentElement.id ? parentElement.id = "dhonishow_"+(index+1) : "";

						var names = names.match(/(\w+|\w+-\w+)_(\w+)/g) || [];
